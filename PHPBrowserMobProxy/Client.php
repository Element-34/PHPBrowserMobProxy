...
require_once '/Users/adam/work/Requests/library/Requests.php';
...
class PHPBrowserMobProxy_Client {
  function __construct($url) {
    ...
    $response = Requests::post($this->host . "/proxy/");
  ...
  function __get($property) {
    switch($property) {
      case "har":
        ...
        curl_setopt($proxy_handle, CURLOPT_URL, $this->host . "/proxy/" . $this->port . "/har");
  ...
  function new_har($label = '') {
    ...
    $response = Requests::put($this->host . "/proxy/" . $this->port . "/har",
                              array(),
                              $data);
  }
  ...
  function new_page($label = '') {
    $data = "pageRef=" . $label;
    $response = Requests::put($this->host . "/proxy/" . $this->port . "/har/pageRef",
                              array(),
                              $data);
  }
  ...
  function whitelist($regexp, $status_code) {
    ...
    $response = Requests::put($this->host . "/proxy/" . $this->port . "/whitelist",
                              array(),
                              $data);
  }
  ...
  function blacklist($regexp, $status_code) {
    $data = $this->encode_array(array("regex" => $regexp, "status" => $status_code));
    $response = Requests::put($this->host . "/proxy/" . $this->port . "/blacklist",
                              array(),
                              $data);
  }
  ...
  function limits($options) {
    ...
    $response = Requests::put($this->host . "/proxy/" . $this->port . "/limit",
                              array(),
                              $data);
  }
  
  function close() {
    $response = Requests::delete($this->host . "/proxy/" . $this->port);
  }
}
